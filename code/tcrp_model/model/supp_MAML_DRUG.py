...
train_feature, train_label, tissue_index_list, drug_test_feature, drug_test_label, _ = load_data_cell_line(
    drug_tissue_map, args.drug, args.tissue, K, path=data_dic)
...
def zero_shot_test(unseen_test_loader):
    ...
    mtest_loss, mtest_pear_corr, mtest_spear_corr, test_prediction, test_true_label = evaluate_new(unseen_tissue_model,
                                                                                                   unseen_test_loader,
                                                                                                   0)
...
for trial in range(num_trials):
    ...
    for k in range(1, K + 1):
        ...
        train_data = np.load(
            test_data_path + '{}_{}_{}-shot_{}-trial_train.npz'.format(args.drug, args.tissue, k, trial))
...
for epoch in range(args.num_updates):
    ...
    zero_test_loss, zero_test_corr, zero_test_spear_corr, test_prediction, test_true_label = zero_shot_test(
        zero_test_data_list)
    ...
    for k in range(1, K + 1):
        ...
        for i in range(num_trials):
            ...
            tissue_train_loss[i], pear_train, tissue_test_loss[i], pear_test, test_prediction, test_true_label, \
            tissue_train_corr[i], tissue_test_corr[i] = unseen_tissue_learn(unseen_train_loader_list[i][k - 1],
