...
# Load data
train_feature, train_label, tissue_index_list, drug_test_feature, drug_test_label, _ = load_data(drug_tissue_map,
                                                                                                 args.tissue, args.drug)
...
train_loss, train_corr, train_spear_corr = np.zeros((args.num_updates, K)), np.zeros((args.num_updates, K)), np.zeros(
    (args.num_updates, K))
test_loss, test_corr, test_spear_corr = np.zeros((args.num_updates, K)), np.zeros((args.num_updates, K)), np.zeros(
    (args.num_updates, K))
...
for trial in range(num_trials):
    ...
    for k in range(1, K + 1):
        train_index_file = testing_path_suffix + args.tissue + '_' + args.drug + '_train_index_' + str(
            trial) + '_' + str(k) + '.npy'
        test_index_file = testing_path_suffix + args.tissue + '_' + args.drug + '_test_index_' + str(trial) + '_' + str(
            k) + '.npy'

        unseen_train_loader, unseen_test_loader = load_unseen_data_loader(train_index_file, test_index_file,
                                                                          drug_test_feature, drug_test_label, K, trial,
                                                                          batch_size=K)
...
for epoch in range(args.num_updates):
    ...
    for k in range(1, K + 1):
        ...
        print(k, 'Few shot', epoch, 'meta training:', train_loss[epoch][k - 1], train_corr[epoch][k - 1],
              test_loss[epoch][k - 1], test_corr[epoch][k - 1])
    ...
    for i in range(meta_batch_size):
        observed_train_loader, observed_test_loader = get_observed_data_loader(train_feature, train_label,
                                                                               tissue_index_list, K,
                                                                               args.inner_batch_size, args.tissue_num)
    ...
    print('Meta update', epoch, meta_train_loss.mean(), meta_train_corr.mean(), meta_val_loss.mean(),
          meta_val_corr.mean(), 'best epoch', best_epoch)
