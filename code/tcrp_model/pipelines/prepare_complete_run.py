...
if dataset == "GDSC1_PDTC":
    for (i, b) in enumerate(zip(priority_chunked)):
        ...
        with open(drug_input_file, 'w') as f:
            f.write('\n'.join(_drugs))
        if run_mode == "tcrp":
            cmd = ['python', 'code/tcrp_model/pipelines/generate_MAML_job_cv.py', '--dataset', dataset, '--tissue',
                   tissue, '--run_name', run_name, '--drug_list_file', drug_input_file, '--job_id', str(i), '--job',
                   'drugs']
            cmd = ' '.join(cmd)
            os.system(cmd)

        elif run_mode == "baseline":
            cmd = ['python', 'code/tcrp_model/pipelines/generate_baseline_job_cv.py', '--dataset', dataset, '--tissue',
                   tissue, '--run_name', run_name, '--drug_list_file', drug_input_file, '--job_id', str(i), '--job',
                   'drugs', '--fewshot_data_path', fewshot_data_path]
            cmd = ' '.join(cmd)
            os.system(cmd)
        else:
            break
