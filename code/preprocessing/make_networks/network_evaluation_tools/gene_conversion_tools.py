...
def query_constructor(gene_list, exclude_prefixes=None, print_invalid_genes=False):
    ...
    valid_query_genes = [get_identifier_without_prefix(gene) for gene in gene_list if
                         exclude_id(gene, exclude_prefixes) == None]
...
def construct_query_map_table(query_result, query_genes, display_unmatched_queries=False):
    ...
    for match in query_result:
        if match.get('query') in partial_match_genes:
            ...
            if match.get(
                    'entrezgene'):  # If there if an entrez gene, we want that that in string form, otherwise we want None
                matched_data.append(
                    [match.get('query'), match.get('_score'), match.get('symbol'), str(match.get('entrezgene'))])
            else:
                matched_data.append(
                    [match.get('query'), match.get('_score'), match.get('symbol'), match.get('entrezgene')])
    ...
    if weighted:
        ...
        for edge in edgelist_filt2:
            if edge[0] + '+' + edge[1] not in edgelist_filt3_scoremap:
                ...
                edgelist_filt3_scoremap[edge[0] + '+' + edge[1]] = max(edgelist_filt3_scoremap[edge[0] + '+' + edge[1]],
                                                                       edge[2])
