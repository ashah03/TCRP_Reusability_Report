...
import pandas as pd
import numpy as np
from . import data_import_tools as dit
from . import network_propagation as prop
from . import network_evaluation_functions as nef
...
def calculate_confusion_matrix_serial(prop_geno, p, n, node_set_name, node_set, verbose=False):
    ...
    for i in range(n):  # Number of times to run the sampling
        ...
        prop_geno_sample_sum = prop_geno.ix[sample][prop_geno_non_sample].sum().sort_values(
            ascending=False)  # summed prop value for all nodes
        ...
        confusion_matrix = {'TP': [], 'FN': [], 'FP': [],
                            'TN': []}  # initialize true positive, false negative, false positive, true negative lists
        for node in intersect_non_sample_sorted:  # Slide down sorted nodes by summed prop value by nodes that are in intersect_non_sample
            TP, FN = sum(y_actual.ix[:node]), sum(
                y_actual.ix[node:]) - 1  # Calculate true positives and false negatives found at this point in list
            FP, TN = len(y_actual.ix[:node]) - TP, len(
                y_actual.ix[node:]) - 1 - FN  # Calculate false positives and true negatives found at this point in list
    ...
    if verbose:
        print('Confusion matrices calculated for node set', node_set_name, 'complete.',
              repr(len(intersect)) + ' nodes in network,', round(time.time() - runtime, 2), 'seconds.')
...
def calculate_confusion_matrix_parallel(node_set_params):
    node_set_name, node_set, p, n, verbose = node_set_params[0], node_set_params[1], node_set_params[2], \
    node_set_params[3], node_set_params[4]
    runtime = time.time()
    intersect = [nodes for nodes in node_set if nodes in prop_geno.index]
    confusion_matrices = {}
    sample_size = int(round(p * len(intersect)))
...
def confusion_matrix_construction_wrapper(network_file, node_set_file, sample_p, sub_sample_iterations,
                                          alpha=None, m=-0.17190024, b=0.7674828, net_delim='\t', set_delim='\t',
                                          cores=1, verbose=False, save_path=None):
    starttime = time.time()
    # Load network
    ...
    if cores == 1:
        ...
        node_set_conf_mat = {
            node_set: nef.calculate_confusion_matrix_serial(prop_net, sample_p, sub_sample_iterations, node_set,
                                                            node_sets[node_set], verbose=verbose) for node_set in
            node_sets}
    else:
        ...
        conf_mat_Analysis_params = [[node_set, node_sets[node_set], sample_p, sub_sample_iterations, verbose] for
                                    node_set in node_sets]
...
def confusion_matrix_analysis(confusion_matrix_input, calculation, recall_threshold=0.9, verbose=False, save_path=None):
    ...
    cohort_calculated_values_table = pd.concat([pd.Series(cohort_calculated_values_mean, name='Average ' + calculation),
                                                pd.Series(cohort_calculated_values_var, name=calculation + ' Var')],
                                               axis=1)
    if save_path is None:
        if verbose:
