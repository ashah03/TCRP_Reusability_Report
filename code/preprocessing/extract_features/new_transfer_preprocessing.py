...
# !wget -P ../gCSI https://zenodo.org/record/4737437/files/gCSI2.rds?download=1
...
# drugs_legend = pd.read_csv('/GDSC/screened_coumpounds.csv', sep=',', index_col=0)
# drug2id_mapping = {}
...
drug_target_map = {k: v for k, v in list(drug_target_map.items()) if v != ['']}
...
for drug, target_list in list(drug_target_map.items()):
...
available_PDTC_drugs = [i for i in PDTC_drugs["drug"].tolist() if i in list(drug2id_mapping.keys())]
PDTC_drugs["DRUG_ID"] = [drug2id_mapping[i] if i in list(drug2id_mapping.keys()) else i for i in
                         PDTC_drugs["drug"].tolist()]
...
for tissue, cell_line in list(tissue_map.items()):
...
for drug in selected_drug_list:
    ...
    if drug not in drug2id_mapping:
        ...
        for tissue, tissue_cell_line_list in list(tissue_map.items()):
        ...
        file_handle = open("/GDSC1_UHNBreast/" + drug + '_tissue_cell_line_list.pkl', "wb")
...
drug_feature_dir = os.listdir("/GDSC1_UHNBreast/drug_feature/")
pickle_files = os.listdir("/GDSC1_UHNBreast/")
...
for i in pickle_files:
    if "pkl" in i:
        ...
        if drug not in drug_feature_dir:
            print(drug)
            os.system(f"rm /GDSC1_UHNBreast/{i}")
